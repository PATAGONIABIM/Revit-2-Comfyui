...
ImageMMFWriter::ImageMMFWriter() : hMapFile(NULL), pBuf(nullptr), header(nullptr), width(0), height(0) {}
...
bool ImageMMFWriter::Initialize(int w, int h) {
    ...
    if (requiredSize > IMAGE_MMF_SIZE) {
        std::cerr << "[MMF Writer] Error: Resolución demasiado alta para el buffer de " << IMAGE_MMF_SIZE / (1024*1024) << "MB" << std::endl;
        return false;
    }
    ...
    if (hMapFile == NULL) {
        std::cerr << "[MMF Writer] Could not create file mapping object (" << GetLastError() << ")." << std::endl;
        return false;
    }
    ...
    if (pBuf == NULL) {
        std::cerr << "[MMF Writer] Could not map view of file (" << GetLastError() << ")." << std::endl;
    ...
    std::cout << "[MMF Writer] Initialized. Size: " << requiredSize / 1024 << " KB" << std::endl;
    return true;
}
                                ...
                                const std::vector<float>& segData) 
{
    ...
    if(!depthData.empty()) memcpy(pDepth, depthData.data(), header->depthSize);
    if(!normalData.empty()) memcpy(pNormal, normalData.data(), header->normalSize);
    if(!linesData.empty()) memcpy(pLines, linesData.data(), header->linesSize);
    if(!segData.empty()) memcpy(pSeg, segData.data(), header->segmentationSize);
